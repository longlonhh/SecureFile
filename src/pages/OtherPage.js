...
export default function OtherPage({ user, onLogout, currentPage, onSidebarClick }) {
  ...
  const otherFiles = uploadedFiles.filter((file) => file.type === "OTHER");
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedFiles(otherFiles.map((_, index) => index));
    }
  ...
  const handleDeleteSelected = () => {
    if (selectedFiles.length > 0) {
      const updatedFiles = uploadedFiles.filter((_, globalIndex) => {
        const localIndex = otherFiles.findIndex(file => file === uploadedFiles[globalIndex]);
      ...
      <div className="flex-1 p-6">
        <div className="bg-white rounded-lg shadow-lg p-4">
          ...
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-700">Other</h2>
