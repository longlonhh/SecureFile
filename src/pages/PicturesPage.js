...
export default function PicturePage({ user, onLogout, currentPage, onSidebarClick }) {
  ...
  const pictureFiles = uploadedFiles.filter((file) => file.type === "IMAGE");
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedFiles(pictureFiles.map((_, index) => index));
    }
  ...
  const handleDeleteSelected = () => {
    if (selectedFiles.length > 0) {
      const updatedFiles = uploadedFiles.filter((_, globalIndex) => {
        const localIndex = pictureFiles.findIndex(file => file === uploadedFiles[globalIndex]);
      ...
      <div className="flex-1 p-6">
        <div className="bg-white rounded-lg shadow-lg p-4">
          ...
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-700">Pictures</h2>
